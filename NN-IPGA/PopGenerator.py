...
class PopGenerator(object):
    ...
    def create_population(self, pop_size, n, m, min_nodes, max_nodes):
        ...
        base_route = self.nearest_neighbor(self.graph)
        pop_routes = []
        for i in range(pop_size):
            tmp_route = copy.deepcopy(base_route)
            divide = int(n/m)
            pos = random.randint(0,n-divide)
            tmp = tmp_route[n:n+divide]
            del tmp_route[n:n+divide]
            random.shuffle(tmp)
            tmp_route[n:n] = tmp
    ...
    def generate_breaks_fully_random(n, m, min, max_nodes=None):
        breakpoints = sorted(random.sample(range(2,n), m-1))
        return breakpoints
